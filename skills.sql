...
--    name for all models made in 1964 from the Models table.
    ...
    SELECT name, brand_name FROM Models WHERE year = 1964;

-- 4. Select the model name, brand name, and headquarters for the Ford Mustang 
--    from the Models and Brands tables.

    SELECT m.name, b.name, b.headquarters FROM Models AS m JOIN Brands AS b WHERE m.name = "Mustang" AND b.name = "Ford";
...
-- 8. Select rows 15-25 of the DB in alphabetical order by model name.
    ...
    SELECT * FROM models GROUP BY name LIMIT 10 OFFSET 14
...
--    the brand is not in the Brands table.)
    ...
    SELECT m.brand_name, m.name, b.headquarters FROM Models AS m LEFT JOIN Brands AS b ON b.name = m.brand_name WHERE year = 1960
